...
err_t Insert(int* arr, uint32_t n, uint32_t maxSz, int elem, uint32_t index)
{
    if(index > n) return ERR_INVALID_INDEX;
    if(n > maxSz) return ERR_BUFFER_OVERFLOW;
    if(maxSz == 0 || n == 0) return ERR_BUFFER_UNDERFLOW;
...
int main(void)
{
    ...
    scanf("%d", &elem);
    uint32_t index;
    printf("index to be inserted at: ");
    scanf("%u", &index);

    err = Insert(arr, n, ARR_MAX_SZ, elem, index);
    if(err != ERR_NONE) goto errHandle;
    ++n;
